...
def _cache_scope_for_request(body: QueryRequest, current_user: User) -> str | None:
    if body.matter_id or body.session_id:
        return None
...
async def query(
    ...
    cache_scope = _cache_scope_for_request(body, current_user)

    if not body.stream and cache_scope:
        cached = await get_cached_response(
            clean_query, query_mode, cache_scope=cache_scope
        )
        if cached:
    ...
    if cache_scope:
        await set_cached_response(
            clean_query, result, query_mode, cache_scope=cache_scope
        )
